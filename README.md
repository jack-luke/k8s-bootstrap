...
### Vault Certificates
Valid Vault server certificates can be generated by following a guide like this: [Cert Manager: Vault Issuer Docs](https://cert-manager.io/docs/configuration/vault/#accessing-a-vault-server-with-mtls-enforced).

Also take note of the name used to create the Vault server leaf certificate, as
this is the SNI name required for some clients to properly verify the Vault 
server.
